...
function CTAButton({ href, disabled, label, size = "md" }: {
  href: string;
  disabled: boolean;
  label: string;
  size?: "md" | "lg";
}) {
  const padCls = size === "lg" ? "px-6 py-4 text-base sm:text-lg" : "px-5 py-3.5 text-[15px] sm:text-base";
  if (disabled) {
    return (
      <span
        aria-disabled
        className={`flex w-full items-center justify-center rounded-xl bg-[var(--surface-2)] font-bold text-[var(--text-subtle)] ${padCls}`}
      >
        {label}
      </span>
    );
  ...
  return (
    <a
      ...
      className={`group flex w-full items-center justify-center gap-2 rounded-xl bg-[#1663de] font-bold text-white shadow-[0_8px_28px_-8px_rgba(22,99,222,0.6)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#0f4cb3] hover:shadow-[0_16px_32px_-8px_rgba(22,99,222,0.7)] ${padCls}`}
    >
      {label}
...
function PriceBox({ product, large = false }: { product: Product; large?: boolean }) {
  ...
  const ctaLabel = product.digistoreUrl
    ? "Jetzt zum Einführungspreis sichern!"
    : "Bald verfügbar — Link folgt";
  const ctaDisabled = !product.digistoreUrl;
  ...
  return (
    <div className={`rounded-2xl border border-[var(--border)] bg-[var(--surface)] ${large ? "p-5 sm:p-6" : "p-4 sm:p-5"}`}>
      ...
      <div className="mt-5">
        <CTAButton href={ctaHref} disabled={ctaDisabled} label={ctaLabel} size={large ? "lg" : "md"} />
      </div>
    ...
    <main className="min-h-dvh bg-[var(--bg)]">
      ...
      >
        <div className="rounded-2xl border border-[var(--border)] bg-gradient-to-br from-[#1663de]/[0.06] to-[#db6f16]/[0.06] p-6 sm:p-8">
          ...
          <p className="mt-3 text-center text-xs text-[var(--text-subtle)]">
            Sichere Zahlung über Digistore24 · MwSt automatisch
          </p>
        </div>
      </motion.section>
