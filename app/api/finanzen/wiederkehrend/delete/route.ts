/**
 * POST /api/finanzen/wiederkehrend/delete — wiederkehrende Rechnung (Vorlage)
 * löschen (login-geschützt). Stoppt künftige Auto-Erzeugung. Body: { id }
 */
export const runtime = "nodejs";

import { NextRequest, NextResponse } from "next/server";
import { isLoggedIn } from "@/lib/angebot/auth";
import { deleteWiederkehrend } from "@/lib/finanzen/recurring";
import { dbReady } from "@/lib/finanzen/db";
...
export async function POST(req: NextRequest) {
  ...
  const ok = await deleteWiederkehrend(body.id);
