 ...
 *
 * Daten aus content/testimonials.ts. Quelle der Reviews: Google Business Profile
 * (Wohlstandsmarketing). Bei neuen Reviews einfach dort eintragen.
 */

import { useState } from "react";
import { motion } from "framer-motion";
import { testimonials, featuredTestimonials, type Testimonial } from "@/content/testimonials";
import { GOOGLE_REVIEWS_URL } from "@/components/GoogleReviewsBadge";
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="inline-flex items-center gap-1.5 text-[11px] font-medium text-[var(--text-subtle)]">
          {GOOGLE_LOGO_SVG} <span>Google</span>
        </span>
      </div>
      ...
      <div className="flex items-center gap-3 border-t border-[var(--border)] pt-4">
        ...
        <div className="leading-tight">
          ...
          <p className="text-[11.5px] text-[var(--text-subtle)]">{formatDate(t.date)} · Verifizierte Google-Bewertung</p>
        </div>
      </div>
    </a>
  );
    ...
    <div className="rounded-3xl border border-[var(--border)] bg-white p-6 shadow-[0_4px_24px_-12px_rgba(10,10,10,0.08)] sm:p-8">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <Stars count={t.rating} />
          <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[var(--text-subtle)]">
            {GOOGLE_LOGO_SVG} Google
          </span>
        </div>
      </div>
...
export default function Testimonials({
  ...
  const title =
    ...
    (isCompact
      ...
      : "Alle Google-Bewertungen für Wohlstandsmarketing.");
  const sub =
    subline ??
    (isCompact
      ? "Echte Google-Bewertungen. Klick führt direkt zu unserem Google-Profil — verifiziert und unbearbeitet."
      : "Ungekürzt. Verifiziert. Direkt aus dem Google Business Profile.");
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        {isCompact ? (
          <>
            ...
            <div className="mt-8 flex items-center justify-center gap-2 text-[12.5px] font-medium text-[var(--text-muted)] lg:justify-start">
              <a
                href={GOOGLE_REVIEWS_URL}
                target="_blank"
                rel="noopener noreferrer"
